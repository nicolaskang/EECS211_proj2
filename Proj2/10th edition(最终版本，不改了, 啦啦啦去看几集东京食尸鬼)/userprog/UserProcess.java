...
public class UserProcess {
    ...
    public UserProcess() {
        ...
        if(fdlist[0].file == null)
            ...
            if(!FDcounter.containsKey("std0"))
                FDcounter.put("std0", 1);
            else
                FDcounter.put("std0", FDcounter.get("std0")+1);
        ...
        if(fdlist[1].file == null)
            ...
            if(!FDcounter.containsKey("std1"))
                FDcounter.put("std1", 1);
            else
                FDcounter.put("std1", FDcounter.get("std1")+1);
        }
    ...
    protected class MemoryAccess {
        protected MemoryAccess(AccessType _accessType, byte[] _data, int _vpn, int _dataStart, int _pageStart, int _len) {
            data = _data;
            vpn = _vpn;
    ...
    private int handleOpen(int a0) {
        ...
        //code makes "cp" syscall failed
        for(int i = 0; i < 16; i++){
            if(fdlist[i].fn.equals(fn)){
                System.out.println(fn+ "Already Opened");
                return i;
            }
        }
    ...
    private int handleClose(int a0) {
        ...
        System.out.println("Now close " + fd.fn);
        fd.file.close();
        fd.file = null;
        //fd.opened = false;
        /*String fn = fd.fn;
         ...
         fd.forRemove = false;*/
        //HashMap<String, Integer> FDcounter
        
        //System.out.println(fd.fn);
        //System.out.println(FDcounter.get(fd.fn));
        Set<String> s = FDcounter.keySet();
        for(String str : s) {
            System.out.println("Process: " + pid + " " +str + " " +FDcounter.get(str));
        }
    ...
    public void HandleExit(int status) {
        ...
        for (int i = 0; i < 16; i++) {
            if (fdlist[i].file != null) {
                Set<String> s = FDcounter.keySet();
                for(String str : s) {
                    System.out.println("Process: " + pid +" " + "In HandleExit: " + str + " " +FDcounter.get(str));
                }
                //System.out.println("Process: " + pid + " " + fdlist[i].file.name+ " exists at index " + i + " ready to close it");
                if (fdlist[i].file != null) {
                    handleClose(i);
                }
            }
