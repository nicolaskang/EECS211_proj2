...
public class UserProcess {
    ...
    public int readVirtualMemory(int vaddr, byte[] data, int offset,
        ...
        Processor processor = Machine.processor();                               /* @Tao  */
        ...
        // calculate virtual page number from the virtual address               /* @Tao */
        int vpn = processor.pageFromAddress(vaddr);                             /* @Tao */
        int addressOffset = processor.offsetFromAddress(vaddr);                 /* @Tao */
        
        TranslationEntry entry = null;                                          /* @Tao */
        entry = pageTable[vpn];                                                 /* @Tao */
        entry.used = true;                                                      /* @Tao */
        
        int ppn = entry.ppn;                                                    /* @Tao */
        int paddr = (ppn*pageSize) + addressOffset;                             /* @Tao */
        // check if physical page number is out of range
        if (ppn < 0 || ppn >= processor.getNumPhysPages())  {                   /* @Tao */
            Lib.debug(dbgProcess,                                               /* @Tao */
                      "\t\t UserProcess.readVirtualMemory(): bad ppn "+ppn);      /* @Tao */
            return 0;                                                           /* @Tao */
        }                                                                       /* @Tao */
    ...
    public int writeVirtualMemory(int vaddr, byte[] data, int offset,
        ...
        entry = pageTable[vpn];                                     /* @Tao */
        entry.used = true;                                          /* @Tao */
        entry.dirty = true;                                         /* @Tao */
        
        int ppn = entry.ppn;                                        /* @Tao */
        int paddr = (ppn*pageSize) + addressOffset;                 /* @Tao */
        
        if (entry.readOnly) {                                       /* @Tao */
            Lib.debug(dbgProcess,                                   /* @Tao */
                      "\t\t [UserProcess.writeVirtualMemory]: write read-only page "+ppn); /* @Tao */
            return 0;                                               /* @Tao */
        }                                                           /* @Tao */
        
        // check if physical page number is out of range
        if (ppn < 0 || ppn >= processor.getNumPhysPages())  {       /* @Tao */
            Lib.debug(dbgProcess, "\t\t [UserProcess.writeVirtualMemory]: bad ppn "+ppn); /* Tao */
            return 0;                                               /* TAO*/
        }
    ...
    protected boolean loadSections() {
        ...
        for (int s=0; s<coff.getNumSections(); s++) {
            ...
            for (int i=0; i<section.getLength(); i++) {
                ...
                TranslationEntry entry = pageTable[vpn];                                   /* Tao */
                entry.readOnly = section.isReadOnly();                                     /* Tao */
                int ppn = entry.ppn;                                                       /* Tao */
                
                section.loadPage(i, ppn);                                                  /* Tao */
            }
        }
