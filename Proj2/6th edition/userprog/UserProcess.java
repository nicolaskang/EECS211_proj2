...
public class UserProcess {
    ...
    public int readVirtualMemory(int vaddr, byte[] data, int offset, int length) {
        ...
        while(length >= 0){
        	//Get physical from virtual memory
            int physicaladdr = translateVirtualToPhysical(currentvaddr);
            if (physicaladdr < 0 || physicaladdr >= memory.length){
                handleException(Processor.exceptionAddressError);
            }
            
            int amount = Math.min(length, Processor.makeAddress(Processor.pageFromAddress(physicaladdr)+1,0) - physicaladdr);
    ...
    private int translateVirtualToPhysical(int address){
    	// get virtual page and offset
        int vpage = Processor.pageFromAddress(address);
        int offset = Processor.offsetFromAddress(address);
        
        // thrown exceptionAddressError if vpage > pageTable.length
        if(vpage >= pageTable.length){
            return -1;
        }
     // thrown exceptionAddressError if vpage is not valid
        TranslationEntry entry = pageTable[vpage];
        if(!entry.valid){
            return -1;
        }
        
        // Get the physical page number at certain vpage
    ...
    public void HandleExit(int status) {
        ...
        if (pid == 0) {
            ...
            for (int i = 0; i < threads.size(); i++) {
            	if (threads.get(i) != null) threads.get(i).finish();
                //KThread.currentThread().finish();
            }
    ...
    public int handleJoin(int id, int sta) {
       ...
       // }
        ...
        int targ = target.getPid();
        //if (target.thread != null) target.thread.join();
        //target.setParent(0);
        
        for (int i = 0; i < target.threads.size(); i++)
        {
            if (target.threads.get(i) != null) target.threads.get(i).join();
        }
        
        int status = exitStat.get(targ);
        exitStat.remove(targ);
        
        if (status == exitexception) return 0;
        
        byte data[] = new byte[4];
        
        data = Lib.bytesFromInt(status);
    ...
    public ArrayList<UThread> threads;
